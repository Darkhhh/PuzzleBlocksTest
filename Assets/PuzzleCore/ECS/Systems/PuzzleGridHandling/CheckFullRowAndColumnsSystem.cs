...
{
    public class CheckFullRowAndColumnsSystem : IEcsRunSystem
    {
        #region ECS Filters
        ...
        public void Run(IEcsSystems systems)
        {
            ...
            var xOffset = (int) firstCell.View.ParentPosition.x;
            var yOffset = (int) firstCell.View.ParentPosition.y;
            var edge = Mathf.RoundToInt((float)Math.Sqrt(_cellsFilter.Value.GetEntitiesCount())) / 2;
            ...
            for (var x = -edge; x <= edge; x += (int)CellView.Size)
            {
                var columnClearingCellsEntities = new List<int>();
                
                for (var y = -edge; y <= edge; y += (int)CellView.Size)
                {
